...
contract DeployHookScript is BaseScript {
    ...
    function run() public {
        ...
        vm.startBroadcast();
        JitLiquidityHook hook = new JitLiquidityHook{salt: salt}(poolManager);
        vm.stopBroadcast();

        require(address(hook) == hookAddress, "DeployHookScript: Hook Address Mismatch");
        
        console.log("JitLiquidityHook deployed at:", address(hook));
        console.log("Vault deployed at:", address(hook.vault()));
    }
}
